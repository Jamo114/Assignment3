...
cmdline_parser::cmdline_parser(void) : vm(), od("Options")
{
	...
	od.add_options()
		...
		((NUMBERTHREADS+",t").c_str(), po::value<int>()->default_value(4),
			"number of threads")
		((XORKEY+",x").c_str(), po::value<std::string>()->default_value("XORKEY"),
			"XOR Key")
		((VIGENEREKEY+",v").c_str(), po::value<std::string>()->default_value("VIGENEREKEY"),
			"Vigenere Key")
		((INITIALIZATIONVECTOR+",I").c_str(), po::value<std::string>()->default_value("ABCDE"),
...
// Return the XOR key
//-------------------------------------------------------------------------//
std::string cmdline_parser::get_xor_key(void) const
{
	// Return whatever value is stored as a string
	return vm[XORKEY].as<std::string>();
}

//-----------------------------------------------------------------------//
// Return the Vigenere key
//-------------------------------------------------------------------------//
std::string cmdline_parser::get_vigenere_key(void) const
{
	// Return whatever value is stored as a string
	return vm[VIGENEREKEY].as<std::string>();
}

//-----------------------------------------------------------------------//
// Return the Initialization vector
//-------------------------------------------------------------------------//
std::string cmdline_parser::get_initialization_vector(void) const
{
	// Return whatever value is stored as a string
	return vm[INITIALIZATIONVECTOR].as<std::string>();
}
